...
![좌측 종목을 클릭하면 우측 뉴스가 그 종목 기사만으로 바뀌는 모습](demo/live.gif)

좌측이 시세, 우측이 뉴스다. 좌측 종목을 클릭하면 우측이 그 종목 뉴스로 바뀌고
(`SK하이닉스 100건`, `지엔씨에너지 12건`) 다시 누르면 전체로 돌아온다.
시세는 10초, 뉴스는 10분마다 갱신된다. 카드는 드래그해서 원하는 자리에 둔다.

![＋ 종목 을 눌러 종목명을 검색하고 결과에서 고르는 모습](demo/add.gif)

`＋ 종목` → 종목명 입력 → 검색 결과에서 선택. 위쪽이 다이얼로그, 아래쪽이 위젯이다.
(이 GIF는 2열로 합치기 전에 찍은 것이라 카드 모양이 다르다. 추가 흐름은 그대로다.)
...
| 뉴스 전체 보기 | 기본 상태가 전체다. 종목을 골랐으면 `‹` 로 복귀 |
...
블록리스트(`NOISE`)로 나머지를 거른다.

### 이름이 애매한 종목 — `NARROW`

`서연`(A007860) 같은 이름은 이름만으로 검색하면 연예인·배구선수 기사가 대부분이다.
그런 종목만 `news.py` 의 `NARROW` 에 코드를 넣으면 증시 맥락으로 좁힌다.

```python
NARROW = {"A007860"}
# → q = "서연" (007860 OR 주가 OR 증권 OR 코스피 OR 코스닥 OR 실적 OR 공시) when:7d
```

**코드를 단독으로 AND 하면 안 된다.** 기사 대부분이 종목코드를 안 적어서 거의 다 날아간다.
그래서 코드를 OR 그룹 안에 넣는다. 실측(`when:30d`):

| 쿼리 | 결과 |
|---|---|
| `"서연"` | 100건 — 대부분 연예인·배구선수 |
| `"서연" 007860` | **1건** — 정확하지만 남는 게 없다 |
| `"서연" (007860 OR 주가 OR …)` | 12건 — 전부 증시 맥락 |
| `"지엔씨에너지"` | 51건 |
| `"지엔씨에너지" 119850` | 10건 — 좋은 기사 41건 손실 |
| `"지엔씨에너지" (119850 OR …)` | 48건 |

**전 종목에 걸면 안 된다.** SK텔레콤이 100 → 52건으로 줄었는데, 빠지는 게 잡소식이 아니라
`SKT '3조 실탄' 확보해 데이터센터 키운다`, `SK텔레콤-아마존 한국 사업 협력` 같은
진짜 회사 뉴스였다. 그래서 기본값은 빈 집합이고, 문제가 생긴 종목만 켠다.

### 보기 방식

카드는 2열이다. 좌측이 시세, 우측이 뉴스. 7일치를 종목당 최대 100건(`PER_STOCK`)
— RSS 한 요청 상한이 100건이라 사실상 **받은 전부** — 모아두고 화면에서 걸러 보여준다.
종목당 요청은 1회뿐이라 수집량을 올려도 **요청 수는 안 늘어난다**.

실측 450건 / 172KB, 위젯 쪽 JSON 파싱 0.7ms. 10초마다 흘려도 부담이 없어서
상한을 걸 이유가 없었다. `LIMIT` 은 종목을 많이 늘렸을 때를 위한 안전장치다.
...
상태는 둘뿐이다. "종목당 최신 1건" 뷰를 따로 뒀더니 `‹` 로 돌아왔을 때
전체인지 요약인지 헷갈려서 없앴다.

선택된 줄은 밝게 표시된다. 같은 줄을 다시 누르거나 `‹` 를 누르면 전체로 돌아온다.
뉴스가 없는 항목(지수·ETF)은 클릭이 안 걸린다. 어느 상태든 **최신순**이다.

> **목록 `key` 는 `name + url` 이어야 한다.** `url` 만 쓰면 안 된다 — 같은 기사가
> 여러 종목에 걸려서(450건 중 15건) key가 중복되고, React 재조정이 깨져 필터를
> 바꿔도 이전 종목 기사가 남는다. 실측으로 `rows=12` 인데 DOM `.item` 이 16개였다.

시각은 하루 안쪽이면 상대시각, 넘으면 날짜를 같이 준다. 7일치가 들어오는데
`162h` 로는 언제 글인지 안 읽히기 때문이다.

```
10분 전  /  7시간 전  /  8/26 · 6일 전
```
